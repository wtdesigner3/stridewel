...
if (isset($_GET['toggle_status']) && isset($_GET['id'])) {
    ...
    if ($conn) {
        mysqli_query($conn, "UPDATE `tbl_faq_categories` SET `status`=$new_st WHERE `id`=$cid");
    }
...
if (isset($_GET['delete'])) {
    ...
    if ($conn) {
        ...
        $cat_q = mysqli_query($conn, "SELECT `name` FROM `tbl_faq_categories` WHERE `id`=$del_id");
        $cat_row = mysqli_fetch_assoc($cat_q);
        if ($cat_row) {
            $cat_name = mysqli_real_escape_string($conn, $cat_row['name']);
            // Count FAQs under this category
            $cnt_q = mysqli_query($conn, "SELECT COUNT(*) as c FROM `tbl_faq` WHERE `category`='$cat_name'");
            $cnt = mysqli_fetch_assoc($cnt_q)['c'] ?? 0;
            if ($cnt > 0) {
                $error = "Cannot delete category '$cat_row[name]' because $cnt FAQ(s) are currently assigned to it. Please reassign those FAQs first.";
            } else {
                mysqli_query($conn, "DELETE FROM `tbl_faq_categories` WHERE `id`=$del_id");
                $msg = "FAQ Category '$cat_row[name]' removed successfully.";
            }
        }
    }
}
...
if (isset($_POST['add_category'])) {
    ...
    if (!empty($name)) {
        ...
        $chk = mysqli_query($conn, "SELECT `id` FROM `tbl_faq_categories` WHERE `name`='$name_esc'");
        if (mysqli_num_rows($chk) > 0) {
            $error = "A category named '$name' already exists.";
        } else {
            $ins = mysqli_query($conn, "INSERT INTO `tbl_faq_categories` (`name`, `slug`, `sort_order`, `status`) VALUES ('$name_esc', '$slug_esc', $sort, $status)");
            if ($ins) {
                $msg = "FAQ Category '$name' added successfully!";
            } else {
...
if (isset($_POST['edit_category'])) {
    ...
    if (!empty($name) && $eid > 0) {
        ...
        // Fetch old name to update FAQs if name changed
        $old_q = mysqli_query($conn, "SELECT `name` FROM `tbl_faq_categories` WHERE `id`=$eid");
        $old_name = mysqli_fetch_assoc($old_q)['name'] ?? '';

        $upd = mysqli_query($conn, "UPDATE `tbl_faq_categories` SET `name`='$name_esc', `slug`='$slug_esc', `sort_order`=$sort, `status`=$status WHERE `id`=$eid");
        if ($upd) {
            // Update linked FAQs if category name changed
            if (!empty($old_name) && $old_name !== $name) {
                $old_esc = mysqli_real_escape_string($conn, $old_name);
                mysqli_query($conn, "UPDATE `tbl_faq` SET `category`='$name_esc' WHERE `category`='$old_esc'");
            }
...
// Fetch all categories with count of FAQs
$categories = [];
$cq = mysqli_query($conn, "SELECT c.*, (SELECT COUNT(*) FROM `tbl_faq` f WHERE f.`category` = c.`name`) as faq_count FROM `tbl_faq_categories` c ORDER BY c.`sort_order` ASC, c.`id` ASC");
if ($cq) {
		...
		<div id="content" class="content">
			...
			<div class="d-flex flex-wrap align-items-center justify-content-between mb-3 pb-2 border-bottom">
				<div>
					<ol class="breadcrumb mb-1">
						...
						<li class="breadcrumb-item"><a href="manage-faq.php">FAQ Management</a></li>
						<li class="breadcrumb-item active">FAQ Categories</li>
					</ol>
					<h1 class="page-header mb-0">
						<i class="fa-solid fa-folder-tree text-warning me-2"></i> FAQ Category Management
					</h1>
				</div>
				<div class="d-flex align-items-center gap-2 mt-2 mt-md-0">
					<a href="manage-faq.php" class="btn btn-outline-secondary px-3">
						<i class="fa-solid fa-circle-question me-1"></i> View All FAQs
					</a>
					<a href="add-faq.php" class="btn btn-warning fw-bold shadow-sm px-3">
						<i class="fa-solid fa-plus-circle me-1"></i> Add FAQ Question
					</a>
				</div>
			</div>
			...
			<div class="cms-subnav-strip mb-4">
				<a href="manage-faq.php" class="cms-subnav-pill">
					<i class="fa-solid fa-list-check"></i> All FAQ Questions
				</a>
				<a href="add-faq.php" class="cms-subnav-pill">
					<i class="fa-solid fa-circle-plus"></i> Add Question
				</a>
				<a href="manage-faq-categories.php" class="cms-subnav-pill active">
					<i class="fa-solid fa-folder-open"></i> FAQ Categories
				</a>
				...
				<div class="col-md-6 col-lg-3">
					<div class="card border-0 shadow-sm rounded-3 p-3">
						<div class="d-flex align-items-center justify-content-between">
							...
							<div class="badge bg-warning-subtle text-warning fs-4 p-2 rounded-3">
					...
					<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
						<div class="card-header bg-white py-3 px-4 border-bottom">
							<h5 class="fw-bold mb-0 text-dark">
								<i class="fa-solid fa-plus text-warning me-2"></i> Create FAQ Category
							</h5>
						</div>
						<div class="card-body p-4">
							<form method="POST">
								<div class="mb-3">
									...
									<input type="text" name="name" class="form-control" placeholder="e.g. Sterilization & Cleaning" required>
									<small class="text-muted">Displayed as an accordion tab on the public FAQ page.</small>
								</div>
								<div class="mb-3">
									...
									<small class="text-muted">Lower numbers appear first on the FAQ page.</small>
								</div>
								...
								<button type="submit" name="add_category" class="btn btn-warning w-100 fw-bold shadow-sm py-2">
									<i class="fa-solid fa-plus-circle me-1"></i> Save FAQ Category
								</button>
							</form>
						</div>
					</div>
				...
				<div class="col-lg-8">
					<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
						<div class="card-header bg-white py-3 px-4 border-bottom d-flex align-items-center justify-content-between">
							<h5 class="fw-bold mb-0 text-dark">
								<i class="fa-solid fa-list text-primary me-2"></i> Existing FAQ Categories (<?= $total_cats ?>)
							</h5>
						</div>
								...
								<table class="table table-hover align-middle mb-0">
									<thead class="table-light">
										<tr>
											...
											<th>Category Name</th>
											<th>Slug</th>
											<th class="text-center" style="width: 110px;">Linked FAQs</th>
										...
										<?php if (!empty($categories)): ?>
											<?php foreach ($categories as $idx => $c): ?>
												<tr>
													...
													<td class="text-center">
														<?php if ($c['status'] == 1): ?>
															<a href="manage-faq-categories.php?toggle_status=1&id=<?= $c['id'] ?>" class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1 rounded-pill text-decoration-none fw-bold" title="Click to Deactivate">
																Active
															</a>
														<?php else: ?>
															<a href="manage-faq-categories.php?toggle_status=0&id=<?= $c['id'] ?>" class="badge bg-secondary-subtle text-secondary border border-secondary-subtle px-2 py-1 rounded-pill text-decoration-none fw-bold" title="Click to Activate">
																Inactive
															</a>
														<?php endif; ?>
													</td>
													<td class="text-end pe-4">
														<div class="btn-group btn-group-sm">
															...
															<a href="manage-faq-categories.php?delete=<?= $c['id'] ?>" class="btn btn-outline-danger btn-sm rounded-2" onclick="return confirm('Delete category \'<?= addslashes(htmlspecialchars($c['name'])) ?>\'?');" title="Delete Category">
										...
										<?php else: ?>
											<tr>
												<td colspan="7" class="text-center py-5 text-muted">
													No FAQ categories found. Add your first category using the form on the left.
												</td>
											</tr>
										<?php endif; ?>
									</tbody>
				...
				<form method="POST">
					...
					<div class="modal-header bg-white py-3 px-4 border-bottom">
						<h5 class="modal-title fw-bold text-dark">
							<i class="fa-solid fa-pen-to-square text-primary me-2"></i> Edit FAQ Category
						</h5>
