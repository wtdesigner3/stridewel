...
// =========================================================================
// 2. FALLBACK IN-MEMORY CATALOG DATA (Offline / Maintenance Redundancy)
// =========================================================================
...
/**
 * Comprehensive Database Schema Synchronizer & Self-Healing Migration
 * Guarantees all tables and columns exist on live database to prevent SQL errors.
 */
function ensure_admin_database_schema($conn = null) {
    if (!$conn) {
        global $conn;
    }
    ...
    try {
        ...
        foreach ($about_cols as $col => $def) {
            if (!in_array(strtolower($col), $existing_about_cols)) {
                @mysqli_query($conn, "ALTER TABLE `tbl_about` ADD `$col` $def");
            }
        }

        // 2. Ensure tbl_contact columns exist
        $contact_cols = [
        ...
        foreach ($contact_cols as $col => $def) {
            if (!in_array(strtolower($col), $existing_contact_cols)) {
                @mysqli_query($conn, "ALTER TABLE `tbl_contact` ADD `$col` $def");
            }
        }

        // Sync legacy con_phone1 / con_address into primary_phone / office_address if blank
        @mysqli_query($conn, "UPDATE `tbl_contact` SET 
            `primary_phone` = COALESCE(NULLIF(`primary_phone`, ''), `con_phone1`, '+91 98100 46038'),
            `secondary_phone` = COALESCE(NULLIF(`secondary_phone`, ''), `con_phone2`, '+91 98100 46038'),
            `primary_email` = COALESCE(NULLIF(`primary_email`, ''), `con_email1`, 'stridewel@gmail.com'),
            `secondary_email` = COALESCE(NULLIF(`secondary_email`, ''), `con_email2`, 'stridewel@gmail.com'),
            `whatsapp_number` = COALESCE(NULLIF(`whatsapp_number`, ''), `con_whatsaap`, '+919810046038'),
            `office_address` = COALESCE(NULLIF(`office_address`, ''), `con_address`, '26-A, 2nd Floor, DLF Industrial Area, Moti Nagar, New Delhi-110015'),
            `google_map_iframe` = COALESCE(NULLIF(`google_map_iframe`, ''), `con_map`),
            `working_hours` = COALESCE(NULLIF(`working_hours`, ''), 'Mon – Sat: 09:30 – 18:30 IST')
            WHERE `con_id`=1");

        // 3. Create tbl_timeline_meta & tbl_timeline
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_timeline_meta` (
        ...
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_timeline` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        // 4. Create tbl_home_trust
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_home_trust` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
            `title` VARCHAR(255) NOT NULL,
        ...
        // 5. Create tbl_home_why_meta & tbl_home_why
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_home_why_meta` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_home_why` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
            `title` VARCHAR(255) NOT NULL,
            `description` TEXT NOT NULL,
        ...
        // 6. Create tbl_home_pipeline_meta & tbl_home_pipeline
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_home_pipeline_meta` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_home_pipeline` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        // 7. Create tbl_faq_categories & tbl_blog_categories
        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_faq_categories` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        if ($fc_cnt == 0) {
            ...
            ('Cryogenic Storage & Handling', 'cryogenic-storage-handling', 3, 1),
            ('Burdizzo Castrators & Surgical', 'burdizzo-castrators-surgical', 4, 1),
            ('Ordering, Bulk Supply & Export', 'ordering-bulk-supply-export', 5, 1)");
        }

        @mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `tbl_blog_categories` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
            `name` VARCHAR(255) NOT NULL,
            `slug` VARCHAR(255) NOT NULL,
            `sort_order` INT DEFAULT 0,
        ...
        if ($bc_cnt == 0) {
            ...
            ('Veterinary Surgery', 'veterinary-surgery', 3, 1),
            ('Livestock Care', 'livestock-care', 4, 1)");
        }

        // 8. Ensure tbl_enquiry columns exist
        $chk_sf = @mysqli_query($conn, "SHOW COLUMNS FROM `tbl_enquiry` LIKE 'source_form'");
        if (!$chk_sf || @mysqli_num_rows($chk_sf) == 0) {
        ...
        if (!$chk_ip || @mysqli_num_rows($chk_ip) == 0) {
            @mysqli_query($conn, "ALTER TABLE `tbl_enquiry` ADD `ip_address` VARCHAR(60) DEFAULT NULL");
        }

        // 9. Ensure supplemental CMS tables exist for any unlisted admin modules
        $supplemental_tables = [
            "CREATE TABLE IF NOT EXISTS `tbl_client` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` varchar(255) DEFAULT NULL, `image` varchar(255) DEFAULT NULL, `sort` int(11) DEFAULT 0, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_teams` (`tt_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `tt_name` varchar(255) DEFAULT NULL, `tt_designation` varchar(255) DEFAULT NULL, `tt_image` varchar(255) DEFAULT NULL, `tt_detail` text DEFAULT NULL, `tt_sort` int(11) DEFAULT 0, `tt_status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_service` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` varchar(255) DEFAULT NULL, `slug` varchar(255) DEFAULT NULL, `image` varchar(255) DEFAULT NULL, `desc` text DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_service_category` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` varchar(255) DEFAULT NULL, `slug` varchar(255) DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_award` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `title` varchar(255) DEFAULT NULL, `image` varchar(255) DEFAULT NULL, `sort` int(11) DEFAULT 0, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_acheivements` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `title` varchar(255) DEFAULT NULL, `count` varchar(50) DEFAULT NULL, `icon` varchar(100) DEFAULT NULL, `sort` int(11) DEFAULT 0, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_career` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `title` varchar(255) DEFAULT NULL, `desc` text DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_case_study` (`b_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `b_title` varchar(255) DEFAULT NULL, `b_image` varchar(255) DEFAULT NULL, `b_detail` text DEFAULT NULL, `b_status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_gallery` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `title` varchar(255) DEFAULT NULL, `image` varchar(255) DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_news` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `title` varchar(255) DEFAULT NULL, `image` varchar(255) DEFAULT NULL, `desc` text DEFAULT NULL, `date` date DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_banner` (`bnr_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `bnr_title` varchar(255) DEFAULT NULL, `bnr_image` varchar(255) DEFAULT NULL, `bnr_status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_breadcrumb` (`brd_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `brd_title` varchar(255) DEFAULT NULL, `brd_image` varchar(255) DEFAULT NULL, `brd_status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_quick_links` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `title` varchar(255) DEFAULT NULL, `url` varchar(255) DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_videos` (`b_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `b_title` varchar(255) DEFAULT NULL, `b_url` varchar(255) DEFAULT NULL, `b_status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
            "CREATE TABLE IF NOT EXISTS `tbl_discount` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `code` varchar(50) DEFAULT NULL, `discount` varchar(50) DEFAULT NULL, `status` tinyint(1) DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
        ];
        ...
        error_log("Database schema synchronization error: " . $e->getMessage());
    }
}
